...
void processDronePacket(const uint8_t *payload, int length, int8_t rssi) {
    ...
    if (validDrone) {
        ...
        if (uavIdStr.length() > 0) {
            existingIt = std::find_if(detectedDrones.begin(), detectedDrones.end(),
                [&uavIdStr](const std::pair<const String, DroneDetection>& entry) {
                    return String(entry.second.uavId) == uavIdStr;
                });
        }
...
String getDroneDetectionResults() {
    ...
    for (const auto& entry : detectedDrones) {
        ...
        const uint32_t ageMs = millis() - d.lastSeen;
...

    {
        ...
        while (droneMeshAnnounced.size() > MAX_DRONE_ANNOUNCED) {
            ...
            for (auto it = droneMeshAnnounced.begin(); it != droneMeshAnnounced.end(); ++it) {
                if ((uint32_t)(now - it->second.lastMeshMs) > (uint32_t)(now - oldest->second.lastMeshMs)) oldest = it;
            }
...
void droneDetectorTask(void *pv)
{
    ...
    while ((forever && !stopRequested) ||
        ...
        if ((int32_t)(millis() - nextResultsUpdate) >= 0 || curDroneCount != lastWrittenDroneCount) {
