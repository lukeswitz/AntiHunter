...
struct PmkidBurst {
    ...
    PsramMap<uint64_t, uint64_t> hshkKeys;
    PsramSet<uint64_t> kdeBssids;
    uint32_t lastSeen{};
};
PsramMap<uint64_t, PmkidBurst> g_pmkidBursts;
static constexpr uint32_t PMKID_ORPHAN_SETTLE_MS = 1500;

static uint64_t eapolSessionKey(const uint8_t *bssid, const uint8_t *sta) {
    uint64_t kb = 0, ks = 0;
...
static constexpr uint32_t HSHK_EAPOL_TIMEOUT_MS = 5000;

static const char *hshkUsablePair(const HandshakeReconstruction &r, bool &authorized) {
    const char *challenge = nullptr;
    authorized = false;
    for (size_t i = 0; i < r.fragments.size(); ++i) {
        const HandshakeFragment &a = r.fragments[i];
        for (size_t j = 0; j < r.fragments.size(); ++j) {
            if (i == j) continue;
            const HandshakeFragment &b = r.fragments[j];
            uint32_t dt = (a.ts > b.ts) ? (a.ts - b.ts) : (b.ts - a.ts);
            if (dt > HSHK_EAPOL_TIMEOUT_MS) continue;
            if (a.msgNum == 2 && b.msgNum == 3 && b.replayCtr == a.replayCtr + 1) { authorized = true; return "M2M3"; }
            if (a.msgNum == 3 && b.msgNum == 4 && b.replayCtr == a.replayCtr)     { authorized = true; return "M3M4"; }
            if (a.msgNum == 1 && b.msgNum == 4 && b.replayCtr == a.replayCtr + 1) { authorized = true; return "M1M4"; }
            if (a.msgNum == 1 && b.msgNum == 2 && b.replayCtr == a.replayCtr && !challenge) challenge = "M1M2";
        }
    }
    return challenge;
}
...
std::atomic<bool>     g_jamEnabled{true};
static std::atomic<uint32_t> g_chanRx[15];
...
// Detects self-ID spoof + channel flood, and records command provenance (which
// radio id issued which command) as an audit trail. Injection cannot be told from
// legit ops on a shared channel, so we log the source instead of alerting (0 FP).
// Alerts log LOCALLY only (never re-broadcast — would amplify a flood).
...
void detect_correlateEapolBait(const uint8_t *sta, const uint8_t *apBssid, int8_t rssi, uint8_t channel);
...
void detect_correlateEapolBait(const uint8_t *sta, const uint8_t *apBssid, int8_t rssi, uint8_t channel) {
    ...
    for (auto &w : g_deauthWitness) {
        ...
        bool dstMatch = memcmp(w.dst, sta, 6) == 0;
        bool wDstBcast = (w.dst[0] & w.dst[1] & w.dst[2] & w.dst[3] & w.dst[4] & w.dst[5]) == 0xFF;
        bool bcastBait = wDstBcast && apBssid && memcmp(w.src, apBssid, 6) == 0;
        if (!dstMatch && !bcastBait) continue;
...
static std::atomic<uint32_t> g_lastForgedDeauthMs{0};

struct DeauthPeerMark { uint32_t lastMs; uint32_t lastBurstMs; };
static PsramMap<uint64_t, DeauthPeerMark> g_deauthPeerTs;
static constexpr size_t MAX_DEAUTH_PEER_MAP = 96;
static constexpr uint32_t DEAUTH_ATTRIB_WIN_MS = 8000;

struct DeauthPairEntry { uint32_t winStartMs; uint16_t count; };
static PsramMap<uint64_t, DeauthPairEntry> g_deauthPair;
static constexpr size_t MAX_DEAUTH_PAIR_MAP = 96;
static constexpr uint32_t DEAUTH_PAIR_WIN_MS = 1000;
static constexpr uint16_t DEAUTH_PAIR_THRESH = 12;

static void deauthNotePeer(uint64_t k, uint32_t now, bool burst) {
    if (g_deauthPeerTs.size() >= MAX_DEAUTH_PEER_MAP && g_deauthPeerTs.find(k) == g_deauthPeerTs.end()) {
        uint32_t oldest = UINT32_MAX;
        uint64_t oldestK = 0;
        for (const auto &kv : g_deauthPeerTs) if (kv.second.lastMs < oldest) { oldest = kv.second.lastMs; oldestK = kv.first; }
        g_deauthPeerTs.erase(oldestK);
    }
    DeauthPeerMark &m = g_deauthPeerTs[k];
    m.lastMs = now;
    if (burst) m.lastBurstMs = now;
}

static bool deauthBurstForPeer(uint64_t k, uint32_t now, uint32_t win) {
    auto it = g_deauthPeerTs.find(k);
    return it != g_deauthPeerTs.end() && it->second.lastBurstMs != 0 &&
           (now - it->second.lastBurstMs) < win;
}
...
static void handleEAPOL(const DetectFrameEvent &e) {
    ...
    // uses in its handshake/PMKID capture mode.
    {
        ...
        const uint8_t *baitBssid = (toDsX == 1 && fromDsX == 0) ? a1X
                                 : (toDsX == 0 && fromDsX == 1) ? a2X
                                                                : (e.payload + 16);
        detect_correlateEapolBait(sta, baitBssid, e.rssi, e.channel);
    }
    ...
    if (g_pmkidEnabled.load() && (keyInfo == 0x00CA || keyInfo == 0x00CB)) {
        ...
        if (sigHit) {
            ...
            Serial.printf("[DETECT] PMKID_FORGE tool=MARAUDER src=%s sta=%s keyinfo=0x%04X ch=%u rssi=%d\n",
                          srBuf, bsBuf, (unsigned)keyInfo, (unsigned)e.channel, (int)e.rssi);
    ...
    if (g_hshkEnabled.load() && msgNum >= 1 && msgNum <= 4) {
        bool forced = deauthBurstForPeer(packMac(bssid), now, DEAUTH_ATTRIB_WIN_MS) ||
                      deauthBurstForPeer(packMac(sta), now, DEAUTH_ATTRIB_WIN_MS);
        bool authorized = false;
        const char *pair = nullptr;
        auto sess = g_hshk.find(eapolSessionKey(bssid, sta));
        if (sess != g_hshk.end()) pair = hshkUsablePair(sess->second, authorized);
        if (::g_detectVerbose.load() && pair && !forced) {
            Serial.printf("[VERIFY-HSHK] pair=%s %s bssid=%s sta=%s suppressed (no deauth burst)\n",
                          pair, authorized ? "authorized" : "challenge",
                          macStr(bssid).c_str(), macStr(sta).c_str());
        }
        if (pair && forced) {
            ...
            if (hit == s_hshkInc.end() || (now - hit->second) > 30000) {
                ...
                Serial.printf("[DETECT] HSHK bssid=%s sta=%s pair=%s %s (deauth-forced)\n",
                              ...
                              authorized ? "authorized" : "challenge");
                ::detect_logIncident(String("HSHK:") + macStr(bssid) + ":" + macStr(sta) + ":" + pair, nullptr);
            }
        }
        if (pair && forced && meshEnabled && sentinel_isRunning() && g_meshHshk.load() && meshRateGate("HSHK", 1000)) {
    ...
    bool kdePresent = false;
    {
        ...
        for (int i = 0; i + 22 <= bodyLen; ++i) {
            ...
            for (int j = 0; j < 16; ++j) if (body[i + 6 + j]) { kdePresent = true; break; }
            break;
        }
    }
    ...
    b.bssidTs[bssK] = now;
    b.hshkKeys[bssK] = eapolSessionKey(bssid, src);
    if (kdePresent) b.kdeBssids.insert(bssK);
    uint16_t win = g_pmkidWindow.load();
    for (auto it = b.bssidTs.begin(); it != b.bssidTs.end(); ) {
        if (now - it->second > win) {
            b.hshkKeys.erase(it->first);
            b.kdeBssids.erase(it->first);
            it = b.bssidTs.erase(it);
        } else ++it;
    }
    uint8_t orphaned = 0;
    for (const auto &kv : b.bssidTs) {
        if ((now - kv.second) < PMKID_ORPHAN_SETTLE_MS) continue;
        auto hk = b.hshkKeys.find(kv.first);
        if (hk == b.hshkKeys.end()) continue;
        auto hs = g_hshk.find(hk->second);
        if (hs != g_hshk.end() && (hs->second.seenMask & 0x02) == 0) orphaned++;
    }
    bool fire = orphaned >= g_pmkidMinBssids.load();
    if (fire) {
        ...
        char lineBuf[280];
        snprintf(lineBuf, sizeof(lineBuf),
                 "{\"src\":\"%s\",\"bssid\":\"%s\",\"rssi\":%d,\"ch\":%u,\"ts\":%u,\"distinct_peers\":%u,\"orphan_m1\":%u,\"kde\":%s}",
                 srBuf, bsBuf, (int)ev.rssi, (unsigned)ev.channel, (unsigned)now,
                 (unsigned)b.bssidTs.size(), (unsigned)orphaned,
                 (b.kdeBssids.size() ? "true" : "false"));
        Serial.printf("[DETECT] PMKID_HARVEST sta=%s orphan_m1=%u peers=%u kde=%u\n",
                      srBuf, (unsigned)orphaned, (unsigned)b.bssidTs.size(),
                      (unsigned)b.kdeBssids.size());
...
// Bruce karma_attack.cpp:1285 sendBeaconFrameHelper — hardcoded rogue BSSID
static const uint8_t KARMA_BRUCE_BSSID[6] = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC};
...
static bool beaconHasCsaOrQuiet(const uint8_t *p, uint16_t len) {
    if (len < 36) return false;
    const uint8_t *ie = p + 36;
    uint16_t ieLen = len - 36, off = 0;
    while (off + 2 <= ieLen) {
        uint8_t t = ie[off], l = ie[off + 1];
        if ((size_t)off + 2 + l > (size_t)ieLen) break;
        if (t == 37 || t == 60 || t == 40) return true;
        off += 2 + l;
    }
    return false;
}
...
static const char *classifyBeaconForgery(const uint8_t *p, uint16_t len) {
    ...
    if (memcmp(p + 10, KARMA_BRUCE_BSSID, 6) == 0) return "FORGE_KARMA_BRUCE";
    ...
    if (len > 36) {
        ...
        while (off + 2 <= len) {
            ...
            // CSA IE value = {switch_mode, new_channel, switch_count}; the count is
            // the tell (Marauder WiFiScan.cpp:8523 post_csa = 25 03 01 00 ff).
            if (tag == 0x25 && l >= 3 && p[off + 4] == 0xFF) return "FORGE_CSA_FF";
            off += 2 + l;
        }
    }
    return nullptr;
...
static void handleBeacon(const DetectFrameEvent &e) {
    ...
    // Detect once, alert (rate-gated), then bail out of the whole handler. ---
    {
        ...
        if (g_beaconFloodActive) {
            ...
            if (g_beaconFloodLastEmit == 0 || (tnow - g_beaconFloodLastEmit) > BEACON_FLOOD_COOLDOWN) {
                ...
                const char *floodTool = classifyBeaconForgery(e.payload, e.len);
                if (!floodTool) floodTool = "-";
                Serial.printf("[DETECT] BEACON_FLOOD active ch=%u rssi=%d tool=%s\n",
                              (unsigned)e.channel, (int)e.rssi, floodTool);
                ::detect_logIncident(String("BEACON_FLOOD:ch") + String(e.channel) + ":" + String((int)e.rssi) + "dBm", nullptr);
                char lb[200];
                snprintf(lb, sizeof(lb),
                         "{\"distinct_bssid\":%u,\"win_ms\":%u,\"ch\":%u,\"rssi\":%d,\"reason\":\"BEACON_FLOOD\",\"tool\":\"%s\",\"ts\":%u}",
                         (unsigned)BEACON_FLOOD_DISTINCT, (unsigned)BEACON_FLOOD_WIN_MS,
                         (unsigned)e.channel, (int)e.rssi, floodTool, (unsigned)tnow);
                logEventToSD("/eviltwin.jsonl", String(lb));
                if (floodTool[0] != '-') {
                    uint16_t fbi = (uint16_t)e.payload[32] | ((uint16_t)e.payload[33] << 8);
                    emitBeaconForgery(bssid_early, "", fbi, 0, e.rssi, e.channel, floodTool);
                }
            ...
            // Flood suppression must not hide a channel-switch / quiet DoS: those are
            // two integer reads and the emit below is rate-gated per BSSID.
            if (!beaconHasCsaOrQuiet(e.payload, e.len)) return;
        }
    }
        ...
        if (ssidSelf[0] && isSelfSsid(ssidSelf) && !isSelfMac(bssid) && !isSelfMac(p + 10)) {
            ...
            if (sit == s_selfCloneSeen.end() || (nowc - sit->second) > 30000) {
                ...
                snprintf(bb, sizeof(bb), "%02X:%02X:%02X:%02X:%02X:%02X",
                         bssid[0],bssid[1],bssid[2],bssid[3],bssid[4],bssid[5]);
                Serial.printf("[DETECT] EVILTWIN src=%s reason=%s (clone of our AP) ssid=%s rssi=%d ch=%u\n",
                              bb, rsn, ssidSelf, (int)e.rssi, (unsigned)e.channel);
                char lb[220];
                snprintf(lb, sizeof(lb),
    ...
    // KARMA/MANA also advertise the harvested SSIDs by BEACON, not only by probe
    // response (Bruce karma_attack.cpp:1400 sendBeaconFrames, hostapd-mana likewise).
    // Same signature either way: one BSSID carrying multiple distinct SSIDs.
    if (g_karmaEnabled.load() && ssid[0] != 0 && !isSelfSsid(ssid)) {
    ...
    if (anySignal && cooldownOk && !g_beaconFloodActive) {
        ...
        Serial.printf("[DETECT] EVILTWIN src=%s reason=%s ssid=%s ouis=%u rssi=%d ch=%u\n",
                      bs, ev.reason, ev.ssid, (unsigned)collisionOuiCount,
                      (int)ev.rssi, (unsigned)ev.channel);
        String bsStr(bs);
        if (meshWorthy) {
        ...
        for (const auto &kv : g_apBaseline) {
            ...
            if (other.hasOweTransition && strcmp(other.ssid, ssid) == 0) {
                if (memcmp(other.oweTransitionBssid, bssid, 6) != 0) {
                    ...
                    Serial.printf("[DETECT] OWE_ABUSE src=%s owe_ap=%s ssid=%s rssi=%d ch=%u\n",
                                  obBuf, owbBuf, safeSsid, (int)e.rssi, (unsigned)e.channel);
...
static void emitBeaconForgery(const uint8_t *bssid, const char *ssid, uint16_t bi,
    ...
    else if (strcmp(reason, "FORGE_KARMA_BRUCE") == 0) { reasonBit = 0x100; reasonIdx = 8; }
    std::lock_guard<std::recursive_mutex> lk(g_mtx);
    ...
    static uint32_t s_lastEmitMs[9] = {0};
...
static void handleDeauthFrame(const DetectFrameEvent &e) {
    ...
    if (forgeFingerprint) g_lastForgedDeauthMs.store(millis());
    if (::g_detectVerbose.load()) {
    ...
    uint32_t now = millis();
    std::lock_guard<std::recursive_mutex> lk(g_mtx);
    bool pairBurst = false;
    {
        uint64_t pairK = packMac(src) ^ (packMac(dst) * 0x9E3779B97F4A7C15ULL);
        auto pit = g_deauthPair.find(pairK);
        if (pit == g_deauthPair.end() || (now - pit->second.winStartMs) > DEAUTH_PAIR_WIN_MS) {
            if (g_deauthPair.size() >= MAX_DEAUTH_PAIR_MAP) {
                uint32_t oldest = UINT32_MAX;
                uint64_t oldestK = 0;
                for (const auto &kv : g_deauthPair) if (kv.second.winStartMs < oldest) { oldest = kv.second.winStartMs; oldestK = kv.first; }
                g_deauthPair.erase(oldestK);
            }
            DeauthPairEntry ne{};
            ne.winStartMs = now;
            ne.count = 1;
            g_deauthPair[pairK] = ne;
        } else {
            DeauthPairEntry &pe = pit->second;
            if (pe.count < 65535) pe.count++;
            pairBurst = pe.count >= DEAUTH_PAIR_THRESH;
        }
    }
    bool attribBurst = pairBurst || forgeFingerprint;
    if (!selfSrc || forgeFingerprint) {
        detect_witnessDeauth(src, dst, e.rssi, e.channel);
        deauthNotePeer(packMac(src), now, attribBurst);
        deauthNotePeer(packMac(p + 16), now, attribBurst);
        if (!isBroadcast) deauthNotePeer(packMac(dst), now, attribBurst);
    }
    ...
    if (it == g_deauthRate.end() || (now - it->second.winStartMs) > DEAUTH_FLOOD_WIN_MS) {
        ...
        bool suppressSelfBurst = selfSrc && sentinelHop && !forgeFingerprint &&
                                 (r.count < (DEAUTH_FLOOD_THRESH * 2));
...
static void handleAssocReq(const DetectFrameEvent &e) {
    ...
    Serial.printf("[DETECT] ASSOC_SLEEP bssid=%s distinct_src=%u frames=%u ch=%u rssi=%d\n",
                  bsBuf, (unsigned)w.distinctSrc.size(), (unsigned)w.frames,
                  (unsigned)w.channel, (int)w.bestRssi);
...
struct ProbeGlobalWin {
    uint32_t winMs{};
    uint16_t total{};
    bool alerted{};
    PsramMap<uint64_t,uint16_t> srcCount;
    PsramMap<uint64_t,uint8_t> srcStreak;
    uint32_t lastEmitMs{};
};
static ProbeGlobalWin g_probeGlobal;
static constexpr uint32_t PROBE_FLOOD_QUIET_MS = 60000;
static constexpr uint8_t  PROBE_SINGLE_MIN_STREAK = 2;
static constexpr size_t   MAX_PROBE_STREAK_MAP = 32;
static void probeGlobalCheck(const uint8_t *src, int8_t rssi, uint8_t channel, uint32_t now) {
    if (now - g_probeGlobal.winMs > 5000) {
        uint16_t thr = g_probeSingleMacThresh.load();
        for (auto it = g_probeGlobal.srcStreak.begin(); it != g_probeGlobal.srcStreak.end(); ) {
            auto c = g_probeGlobal.srcCount.find(it->first);
            if (c == g_probeGlobal.srcCount.end() || c->second < thr) it = g_probeGlobal.srcStreak.erase(it);
            else ++it;
        }
        for (const auto &kv : g_probeGlobal.srcCount) {
            if (kv.second < thr) continue;
            if (g_probeGlobal.srcStreak.size() >= MAX_PROBE_STREAK_MAP &&
                g_probeGlobal.srcStreak.find(kv.first) == g_probeGlobal.srcStreak.end()) continue;
            uint8_t &s = g_probeGlobal.srcStreak[kv.first];
            if (s < 255) s++;
        }
    ...
    bool singleMac = sc >= g_probeSingleMacThresh.load() &&
                     st != g_probeGlobal.srcStreak.end() && st->second >= PROBE_SINGLE_MIN_STREAK;
    if (!randomized && !singleMac) return;
    g_probeGlobal.alerted = true;
    if (g_probeGlobal.lastEmitMs != 0 && (now - g_probeGlobal.lastEmitMs) < PROBE_FLOOD_QUIET_MS) return;
    g_probeGlobal.lastEmitMs = now;
...
static void handleProbeReq(const DetectFrameEvent &e) {
    ...
    size_t ssidLenB = strlen(ssidB);
    bool ssidRandom = ssidLenB && ssidLooksRandom(ssidB, ssidLenB);
    bool ssidOk = ssidB[0] && ssidIsValid(ssidB, ssidLenB) && !ssidRandom;
    if (!ssidRandom) probeGlobalCheck(p + 10, e.rssi, e.channel, millis());
    ...
    // per-source cooldown is useless — gate globally.
    {
        ...
        uint32_t tnow = millis();
        if (s_probeForgeEmit == 0 || (tnow - s_probeForgeEmit) > 30000) {
            s_probeForgeEmit = tnow;
            char sb[18];
            snprintf(sb, sizeof(sb), "%02X:%02X:%02X:%02X:%02X:%02X",
                     p[10],p[11],p[12],p[13],p[14],p[15]);
            Serial.printf("[DETECT] PROBE_FLOOD MARAUDER src=%s ssid=%s seq=0x%04X ch=%u rssi=%d\n",
                          sb, ssidB, seqCtrl, (unsigned)e.channel, (int)e.rssi);
            char lb[220];
            snprintf(lb, sizeof(lb),
                     "{\"src\":\"%s\",\"ssid\":\"%s\",\"tool\":\"MARAUDER\",\"seq\":%u,\"reason\":\"PROBE_TEMPLATE\",\"rssi\":%d,\"ch\":%u,\"ts\":%u}",
                     sb, ssidB, (unsigned)seqCtrl, (int)e.rssi, (unsigned)e.channel, (unsigned)tnow);
            logEventToSD("/probe_flood.jsonl", String(lb));
            ::detect_logIncident(String("PROBE_FLOOD:MARAUDER:") + sb + ":" + ssidB, nullptr);
            quorum_addReport("PROBE_FLOOD", String("MARAUDER"), getNodeId(), e.rssi);
            attacker_kick(p + 10, "PROBE_FLOOD");
            if (meshEnabled && g_meshProbeFlood.load() && meshRateGate("PROBE_FLOOD_MARAUDER", 30000)) {
                sendToSerial1(getNodeId() + ": PROBE_FLOOD:MARAUDER:" + String(sb) +
                              ":" + String((int)e.rssi), true);
            }
        }
    }
...
static void handleProbeResp(const DetectFrameEvent &e) {
    ...
    if (it == g_apBaseline.end()) {
        if (::g_detectVerbose.load())
            Serial.printf("[VERIFY-SCN] no baseline for %s resp_ssid=%s\n", macStr(bssid).c_str(), ssid);
        return;
    }
    ApBaseline &b = it->second;
    if (b.ssid[0] == 0) {
        if (::g_detectVerbose.load())
            Serial.printf("[VERIFY-SCN] baseline has empty ssid for %s\n", macStr(bssid).c_str());
        return;
    }
    if (::g_detectVerbose.load())
        Serial.printf("[VERIFY-SCN] %s beacon_ssid=%s resp_ssid=%s match=%d\n",
                      macStr(bssid).c_str(), b.ssid, ssid, strcmp(b.ssid, ssid) == 0);
    ...
    Serial.printf("[DETECT] SSID_CONFUSION src=%s beacon_ssid=%s resp_ssid=%s rssi=%d ch=%u\n",
                  bsBuf, b.ssid, ssid, (int)e.rssi, (unsigned)e.channel);
...
static void handleAuthSae(const DetectFrameEvent &e) {
    ...
    if (!c.alerted && unmatched >= g_saeUnmatchedThresh.load()) {
        ...
        Serial.printf("[DETECT] SAE_DOS bssid=%s unmatched=%u commits=%u confirms=%u ch=%u rssi=%d\n",
                      bsBuf, (unsigned)unmatched, (unsigned)c.commits, (unsigned)c.confirms,
...
static void handleQosData(const DetectFrameEvent &e) {
    ...
    auto fireFrag = [&](const char *reason, uint32_t lastPN, uint32_t obsPN) {
        ...
        Serial.printf("[DETECT] FRAG src=%s reason=%s prev_pn=%u pn=%u ch=%u\n",
                      srcBuf, reason, (unsigned)lastPN, (unsigned)obsPN, (unsigned)e.channel);
        ...
        if (p.begin("ahdetect", false)) {
            {
                ...
                if (!detMig) {
                    ...
                    p.putBool("scnOn", true);
                    p.putBool("saeOn", true);
...
static void sentinelAlwaysOnTask(void *pv) {
    ...
    bool weOwn = false;
    uint32_t s_appliedMask = 0;
    while (g_sentinelUserEnabled.load() && !scanning.load()) {
        // Mask re-evaluated every pass: toggling jam/hshk/pmkid/frag at runtime must
        // re-apply the filter, or FCSFAIL/DATA never arrive until the task restarts.
        bool wantData = g_pmkidEnabled.load() || g_hshkEnabled.load() || g_fragEnabled.load();
        bool wantFcs  = g_jamEnabled.load();
        uint32_t wantMask = WIFI_PROMIS_FILTER_MASK_MGMT;
        if (wantData) wantMask |= WIFI_PROMIS_FILTER_MASK_DATA;
        if (wantFcs)  wantMask |= WIFI_PROMIS_FILTER_MASK_FCSFAIL;
        if (!weOwn || wantMask != s_appliedMask) {
            wifi_promiscuous_filter_t filter = {};
            filter.filter_mask = wantMask;
            esp_err_t r1 = esp_wifi_set_promiscuous_filter(&filter);
            esp_err_t r2 = ESP_OK, r3 = ESP_OK;
            if (!weOwn) {
                r2 = esp_wifi_set_promiscuous_rx_cb(&sniffer_cb);
                r3 = esp_wifi_set_promiscuous(true);
                weOwn = true;
            }
            s_appliedMask = wantMask;
            Serial.printf("[SENTINEL] filter=MGMT%s%s r1=%d r2=%d r3=%d\n",
                          wantData ? "+DATA" : "", wantFcs ? "+FCSFAIL" : "",
        ...
        static uint32_t s_lastRxReport = 0;
        if (millis() - s_lastRxReport > 10000) {
            s_lastRxReport = millis();
            std::vector<uint8_t> rep;
            {
                std::lock_guard<std::recursive_mutex> lk(g_mtx);
                rep = CHANNELS;
            }
            if (rep.empty() || !g_sentinelScanMode.load()) rep.assign(1, apChan);
            char rb[200];
            int off = snprintf(rb, sizeof(rb), "[SENTINEL] rx/10s");
            for (uint8_t c : rep) {
                if (c > 14 || off >= (int)sizeof(rb) - 12) continue;
                uint32_t n = g_chanRx[c].exchange(0, std::memory_order_relaxed);
                off += snprintf(rb + off, sizeof(rb) - off, " %u:%u", (unsigned)c, (unsigned)n);
            }
...
void IRAM_ATTR detect_onWifiFrame(const uint8_t *payload, uint16_t len, int8_t rssi, uint8_t channel) {
    ...
    if (len < 24) return;
    if (channel < 15) g_chanRx[channel].fetch_add(1, std::memory_order_relaxed);
